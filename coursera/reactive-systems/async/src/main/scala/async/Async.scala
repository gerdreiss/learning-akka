...
object Async extends AsyncInterface:
  ...
  def sequenceComputations[A, B](
    ...
    for
      computation1 <- makeAsyncComputation1()
      computation2 <- makeAsyncComputation2()
    yield (computation1, computation2)
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] =
    makeAsyncComputation()
      .recoverWith {
        case NonFatal(e) => 
          if (maxAttempts == 1) Future.failed(e)
  ...
  def futurize(callbackBasedApi: CallbackBasedApi): FutureBasedApi =
    val p = Promise[Int]()
    callbackBasedApi.computeIntAsync(r => p.tryComplete(r))
    new FutureBasedApi {
